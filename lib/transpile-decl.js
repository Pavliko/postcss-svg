...
function paramsFromNodes(nodes) {
	...
	nodes.forEach(
		(node) => {
			...
			if (node.type === 'function' && node.value === 'param' && node.nodes.length === 3) {
				var keyNode = node.nodes[0];

				if (keyNode.type === 'word') {
					var valueNode = node.nodes[2];

					if (valueNode.type === 'word') {
						// Basic word params and hex values
						params[keyNode.value] = valueNode.value;
					}
					else if (valueNode.type === 'function' && (valueNode.value === 'rgb' || valueNode.value === 'rgba' || valueNode.value === 'hsl' || valueNode.value === 'hsla')) {
						// Function-style color values
						params[keyNode.value] = getFunctionValue(valueNode);
					}
				}
			}
		}
...
// get the string representation of the passed function node
function getFunctionValue(node) {
